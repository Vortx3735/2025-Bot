...
public class DriveToPointCommand extends Command {
  ...
  public DriveToPointCommand(
      ...
      Constraints translationContraints,
  ...
  public void execute() {
    ...
    // System.out.println(currPose + " " + targetPoint + " " + translationToTarget + " " +
    // distanceRemaining);
    ...
    Translation2d driveTranslation =
        translationToTarget.div(translationToTarget.getNorm()).times(driveVelocity);

    ChassisSpeeds newChassisSpeeds =
        new ChassisSpeeds(driveTranslation.getX(), driveTranslation.getY(), 0);

    swerve.runVelocity(
        ChassisSpeeds.fromFieldRelativeSpeeds(newChassisSpeeds, swerve.getRotation()));
